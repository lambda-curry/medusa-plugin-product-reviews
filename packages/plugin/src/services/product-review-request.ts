...
class ProductReviewRequestService extends TransactionBaseService {
  ...
  constructor(container: InjectedDependencies) {
    ...
    this.productReviewRequestRepository_ =
      container.productReviewRequestRepository;
    this.orderService = container.orderService;
  }

  async create(order_id: string) {
    return await this.atomicPhase_(async manager_ => {
      const repo = manager_.withRepository(
        this.productReviewRequestRepository_
      );
      ...
      const request = await repo.save({ order_id });

      await this.eventBusService
        .withTransaction(manager_)
        .emit(ProductReviewRequestService.Events.CREATED, {
          id: request.id,
  ...
  async delete(id: string) {
    return await this.atomicPhase_(async manager_ => {
      ...
      await this.eventBusService
        .withTransaction(manager_)
        .emit(ProductReviewRequestService.Events.DELETED, { id });
    });
  }
}
