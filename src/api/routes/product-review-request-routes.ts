...
export const routes: RouteConfig[] = [
  {
    requiredAuth: false,
    path: "/store/product-review-request/:id",
    method: "get",
...
async function getClientRequest(req: Request, res: Response) {
  const productReviewRequestService =
    req.scope.resolve<ProductReviewRequestService>(
      "productReviewRequestService"
    )
  const reviewRequest = await productReviewRequestService.retrieve(
    req.params.id
  )
